...
// src/rutas/estudios.ts
var estudiosRouter = new Hono2();
...
// src/rutas/usuarios.ts
var usuariosRouter = new Hono2();
var ROLES_VALIDOS = ["titular", "asociado", "administrativo"];
usuariosRouter.post("/", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "No se pudo crear el usuario. \xBFEl email ya est\xE1 en uso?" }, 409);
  }
...
usuariosRouter.get("/", async (c) => {
  ...
  const query = estudioId ? c.env.DB.prepare(
  ...
  ).bind(estudioId) : c.env.DB.prepare(
    `SELECT id, estudio_id, nombre, apellido, dni, email, rol, activo, creado_en
...
// .wrangler/tmp/bundle-VikR4h/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VikR4h/middleware-loader.entry.ts
